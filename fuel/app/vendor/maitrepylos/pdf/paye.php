    ...
    public static function pdf($formData, $nombre_contrat)
    {
        ...
        for ($z = 0; $z < $nombre_contrat;$z++) {
            ...
            for ($b = 0; $b < $formData[$z]['count']; $b++) {
                ...
                $pdf->SetXY(25, 40);
                $pdf->Cell('', 5, 'Pour le mois : ' . \Maitrepylos\Utils::mois($formData[$z]['mois']) . ' ' . $formData[$z]['annee'], '', '', 'C');
                $pdf->SetXY(25, 45);
                $pdf->Cell('', 5, utf8_decode('Groupe : ' . $formData[$z]['groupe'][0]['t_nom'].' - Lieu de formation : '. $formData[$z]['groupe'][0]['t_lieu']
                . ' - Type de Contrat : ' . $formData[$z]['groupe'][0]['t_type_contrat']), '', '', 'C');
...
//            //$pdf->Text(25, $y + 20, 'soit : ' . $heure[0]->fulltime . utf8_decode(' Heures à ') . $formData[$z]['salaire'][0]->ren_tarif_horaire . ' euro/H pour un total de ' . $formData[$z]['salaire'][0]->total . utf8_decode(' euro'));
                $pdf->Text(25, $y + 5, utf8_decode('(H)eures de présences =  :	' . $time->TimeToString((int)$formData[$z]['totalHeuresMois'][0]['fulltime'])));
            ...
            $pdf->Text(25,$y+17,utf8_decode('Heures de présence depuis le début de la formation :'.$time->TimeToString($formData[$z]['heure_total_formation'][0]['fullTime'])));
                $pdf->Text(25, $y + 20, 'Transport : ' . utf8_decode($formData[$z]['t_moyen_transport']));
                //           $pdf->SetFont('Arial', 'B', 10);
                $pdf->Text(140, $y + 20, utf8_decode('Défraiement : ') . $formData[$z]['salaire'] . utf8_decode(' euro'));
//            //$pdf->SetFont('Arial', 'B', 10);
...
//            {
                ...
                $pdf->Text(25, $y + 31, utf8_decode('Nombres de (j)ours de présences :' . $formData[$z]['nombresPrestations'] . ' jours'));
...
//                $pdf->Text(140, $y + 40, utf8_decode('Déplacement : ') . $formData[$z]['deplacement'][0]->somme . utf8_decode(' euro'));
                $pdf->Text(140, $y + 45, utf8_decode('Ajout déplacement : ') . $formData[$z]['ajoutDeplacemement'] . utf8_decode(' euro'));
                $somme = $formData[$z]['deplacement'] + $formData[$z]['salaire'] + $formData[$z]['ajoutDeplacemement'];
//            } else
//            {
