...
class Etatprestation
{
    ...
    public static function pdf($formData)
    {
        ...
        for ($b = 0; $b < $formData['count']; $b ++) {
            ...
            $pdf->SetXY(55, 12);
            $pdf->Cell(215, 28, '', 1);
            $pdf->Text(58, 18, utf8_decode('Nom de l\'opérateur : '.$formData['xml']->t_denomination));
            ...
            for ($i = 0; $i < 11; $i ++) {
                ...
                $pdf->drawTextBox($formData['rows'][$a]['t_nom'].' '.utf8_decode($formData['rows'][$a]['t_prenom']), 45, 7);
                $pdf->SetXY(50, $y);
            ...
            $pdf->Text(148, 183, utf8_decode($formData['xml']->t_responsable));
