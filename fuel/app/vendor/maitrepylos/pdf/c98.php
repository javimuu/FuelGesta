...
//			$pdf->Text(18, 230, utf8_decode('secrétaire'));
            ...
            $pdf->SetXY(159, 232);
            $pdf->Cell('', '', utf8_decode($xml->t_denomination), '', '', 'C');
            $pdf->SetXY(159, 235);
            $pdf->Cell('', '', utf8_decode($xml->t_objet_social), '', '', 'C');
            $pdf->SetXY(159, 238);
            $pdf->Cell('', '', utf8_decode($xml->t_agregation), '', '', 'C');
            $pdf->SetXY(159, 241);
            $pdf->Cell('', '', utf8_decode($xml->t_agence), '', '', 'C');
            $pdf->SetXY(159, 244);
            $pdf->Cell('', '', utf8_decode($xml->t_adresse) . ' ' . utf8_decode('à') . ' ' . $xml->t_code_postal . ' ' . $xml->t_localite, '', '', 'C');
            $pdf->SetXY(159, 247);
            $pdf->Cell('', '', utf8_decode('Tél:' . $xml->t_telephone) . ' - ' . utf8_decode($xml->t_email), '', '', 'C');
            $pdf->SetXY(159, 250);
            $pdf->Cell('', '', utf8_decode('TVA:' . $xml->t_tva . '- Enregitr.' . utf8_decode($xml->t_enregistrement)), '', '', 'C');
            $pdf->SetXY(15, 256);
