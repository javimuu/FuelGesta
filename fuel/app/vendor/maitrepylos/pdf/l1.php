    ...
    public static function pdf($formData)
    {
        ...
        for($z = 0;$z<$formData['compteur'];$z++){
            ...
            for ($b = 0; $b < $formData['nombre'][$z]; $b ++)
            {
                ...
                $pdf->Text(160, 19, utf8_decode('Doc.L.1(1)'));
                $pdf->SetFont('Arial', 'U', 8);
                $pdf->Text(113, 22, utf8_decode('A laisser à disposition de l\'Administration dans votre organisme'));
                $pdf->SetFont('Arial', 'BU', 10);
                $pdf->SetY(35);
                $pdf->drawTextBox(utf8_decode('EFT : Liste des présences journalières des stagiaires en centre ou chez un
 partenaire (hors stage en entreprise)'), '', '', 'C', 'M');

                $pdf->SetFont('Arial', '', 8);
                $pdf->SetXY(14, 50);
                $pdf->Cell(93, 5, utf8_decode('Nom de l\'organisme agréé : '), 1);
                $pdf->SetXY(107, 50);
                $pdf->Cell(94, 5,$formData['xml']->t_denomination, 1);
                $pdf->SetXY(14, 55);
                ...
                for ($i = 0; $i < 18; $i ++)
                {
                    ...
                    $pdf->SetXY($x + 52, $y);
                    $pdf->Cell(35, 8, utf8_decode($formData['rows'][$z][($a-1)]['t_prenom']), 1);
                    $pdf->SetXY($x + 87, $y);
                    $pdf->Cell(35, 8, '', 1);
                    $pdf->SetXY($x + 122, $y);
                    $pdf->Cell(32, 8, '', 1);
