...
<table class="table form-inline table-striped table-top table-inner-bordered table-condensed">
    ...
    <thead>
    <tr>
        <th class="text-center tdark" colspan="9"><?php echo $groupes['tNom']; ?></th>
    </tr>
    <tr>
        <th>Stagiaires</th>
    ...
    <tr>
        <td>
            ...
            <?php echo Html::anchor('participant/modifier/'.$participant['idParticipant'],$participant['tNom'].' '.$participant['tPrenom']); ?>
        </td>
            ...
            <?php foreach ($participant['contrat'] as $contrat): ?>
                <?php if ($contrat[0] !== false): ?>
                    ...
                    <?php if ($contrat[1] == '+'): ?>
                        ...
                        <td class="text-center">
                            ...
                            $participant['idParticipant'] . '/' . $participant[(string)$i] . '/+/' . $participant['idContrat'][0]['idContrat'] . '/Travail',
                            array('id'=>'check'.$i.$a,'onclick'=>"InactiveCheckBox('chick$i$a','chock$i$a');")) ?>

                            <?php echo Form::label('J', 'action'); ?>
                            <?php echo Form::checkbox('action[]', $date[$i]->format('Y-m-d') . '/' .
                            $participant['idParticipant'] . '/' . $participant[(string)$i] . '/%/' . $participant['idContrat'][0]['idContrat'] . '/Absence à Justifier',
                            array('id'=>'chick'.$i.$a,'onclick'=>"InactiveCheckBox('check$i$a','chock$i$a');")) ?>

                            <?php echo Form::label('A', 'action'); ?>
                            <?php echo Form::checkbox('action[]', $date[$i]->format('Y-m-d') . '/' .
                            $participant['idParticipant'] . '/' . $participant[(string)$i] . '/*/' . $participant['idContrat'][0]['idContrat'] . '/Absence',
                ...
                <?php endforeach; ?>
            <?php endif; ?>
        <td class="text-center"> <?php echo Html::anchor('prestation/tableau/' . $participant['tNom'] . ' ' . $participant['tPrenom'] . '/'
                . $participant['idParticipant'] . '/' . $date[0]->format('Y-m-d'), '<i class="icon-edit icon-white"></i>',
