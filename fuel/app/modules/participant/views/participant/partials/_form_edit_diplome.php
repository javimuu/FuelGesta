...
<div class="control-group">
    ...
    <div class="controls">
        <?php echo Form::select('t_diplome', Input::post('t_diplome', isset($participant) ? $participant->t_diplome : 'Enseignement ordinaire'), $diplomes); ?>
    </div>
</div>
<div class="control-group">
    <?php echo Form::label('Année étude', 't_annee_etude', array('class' => 'control-label')); ?>
    <div class="controls">
        <?php //echo Form::select('t_annee_etude', Input::post('t_annee_etude', isset($participant) ? $participant->t_annee_etude : ''), Cranberry\MyXML::get_annee_etude(),array("dont_prep" => true)); ?>

        <select name="t_annee_etude">

            <?php if (isset($participant)): ?>

                <option
                    value="<?php echo $participant->t_annee_etude ?>"><?php echo $participant->t_annee_etude ?></option>;

            <?php endif ?>

            <?php foreach (Cranberry\MyXML::get_annee_etude() as $key => $value): ?>
