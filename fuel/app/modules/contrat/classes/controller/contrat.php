...
//    // Selon la doc, à overwriter à la place de __construct()
    ...
    public function action_ajouter($id = NULL)
    {
        ...
        $count = count($this->data['viewcontrat']);
        for($i=0;$i<$count;$i++){
    ...
    public function action_supprimer($id, $id_participant)
    {
        ...
        $db->deleteContrat($id);
            ...
            $message[] = 'vous ne pouvez supprimer un contrat qui posséde des heures de prestation.';
            \Session::set_flash('error', $message);
        }
     //   $success[] = 'Contrat Supprimé !';
     //   \Session::set_flash('success', $success);
    ...
    public function action_modifier($id = null, $id_participant = null)
    {
        ...
        if (Input::method() == 'POST') {
            ...
            /**
             * On vérifie que l'on ne possède pas déjà des heures pour ce contrat qui empècherait la modification du contrat
             */

            if (\Model_My_Contrat::getHourPrecedenteContrat(\Input::post('d_date_debut_contrat'), $id) == false) {
                $message[] = 'Vous essayez de modifier un contrat qui possède déjà des heures en deça de la date d\'entrée ';
                $true = false;
            }

            if (\Model_My_Contrat::getHourSuivantContrat(\Input::post('d_date_fin_contrat_prevu'), $id) == false) {
                $message[] = 'Vous essayez de modifier un contrat qui possède déjà des heures au dela de la date de sortie prévue ';
                $true = false;
            }
    ...
    public function action_fin_formation($id = null, $id_participant = null)
    {
        ...
        if (Input::method() == 'POST') {
            ...
            if (\Model_My_Contrat::getHourSuivantContrat(\Input::post('d_date_fin_formation'), $id) == false) {
                $message[] = 'Vous essayez de terminer un contrat qui possède déjà des heures au dela de la date de sortie prévue ';
                \Session::set_flash('error', $message);
                \Response::redirect('contrat/ajouter/' . $id_participant);
            }
