...
class Model_Heures_Prestation extends Orm\Model
{
    ...
    public static function validate($factory, $id_groupe)
    {
        ...
        $val->add_field('jours_janvier', 'Jours Janvier', 'required|valid_string[numeric]');
        $val->add_field('jours_fevrier', 'Jours Février', 'required|valid_string[numeric]');
        $val->add_field('jours_mars', 'Jours Mars', 'required|valid_string[numeric]');
        $val->add_field('jours_avril', 'Jours Avril', 'required|valid_string[numeric]');
        $val->add_field('jours_mai', 'Jours Mai', 'required|valid_string[numeric]');
        $val->add_field('jours_juin', 'Jours Juin', 'required|valid_string[numeric]');
        $val->add_field('jours_juillet', 'Jours Juillet', 'required|valid_string[numeric]');
        $val->add_field('jours_aout', 'Jours Août', 'required|valid_string[numeric]');
        $val->add_field('jours_septembre', 'Jours Septembre', 'required|valid_string[numeric]');
        $val->add_field('jours_octobre', 'Jours Octobre', 'required|valid_string[numeric]');
        $val->add_field('jours_novembre', 'Jours Novembre', 'required|valid_string[numeric]');
        $val->add_field('jours_decembre', 'Jours Décembre', 'required|valid_string[numeric]');

        return $val;
    }

    public static function validate_jours($factory, $id_groupe = null)
    {
        $val = Validation::forge($factory);
    ...
    {
        $result = array();
        $annee = DB::query('SELECT DISTINCT(annee) FROM heures_prestations ORDER BY annee')->execute();
