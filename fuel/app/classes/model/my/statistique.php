...
class Model_My_Statistique extends \Maitrepylos\db {
    ...
    public function getGroupe(\DateTime $date) {
        $sql = "SELECT DISTINCT(g.id_groupe),t_nom
                ...
                WHERE EXTRACT(YEAR FROM c.d_date_fin_contrat_prevu) >= ?";
        $result = $this->_db->prepare($sql);
        $result->execute(array($date->format('Y')));
...
//    }
    ...
    /**
     * Récupération du total d'heures des années antérieur en cours par filiére pour le trimestrielle.
     * @param $idParticipant
     * @param $date
     * @param $schema
     * @return int
