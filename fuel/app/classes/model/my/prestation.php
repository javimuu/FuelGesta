...
class Model_My_Prestation extends \Maitrepylos\Db {
     ...
     * @return array
     */
    public function getContratFull($idParticipant, \Datetime $date, $nomMois, $majNomMois) {
        $dateFin = new DateTime();
        $dateFin->setDate($date->format('Y'), $date->format('m'), $date->format('t'));


        $sql = "
            SELECT c.id_contrat,g.t_nom,$nomMois as heures ,$majNomMois as jours,subside_id,i_paye,f_tarif_horaire,c.t_moyen_transport
            ,c.t_abonnement
            FROM contrat c
                INNER JOIN groupe g
    ...
    public function select_contrat($id,  \DateTime $date) {
        ...
        $sql = "SELECT id_contrat,t_type_contrat,g.t_nom
                FROM contrat as c
                    INNER JOIN type_contrat as tc
                        ON tc.id_type_contrat = c.type_contrat_id
...
//        );
    ...
    /**
     * S'il y a un ajout de déplacement, pour une raison ou une autre
     * @param $id
     * @param Datetime $date
     * @return int
     */
    public function getAjoutDeplacement($id, \Datetime $date) {
...
//    }
    ...
    public function verifieValide($id, \DateTime $date) {

        $sql = '
            SELECT COUNT(id_valider_heure) as compteur FROM valider_heure
            WHERE t_mois = ?
            AND participant_id = ?';
        $req = $this->_db->prepare($sql);
    ...
    /**
     * @TODO requete en double à vérifier
     * @param $id_participant
     * @param $id_contrat
     * @param Datetime $date
     * @return mixed
     ...
     * @return
     */
    public function getHeureTotalFormation($id, \DateTime $date) {
        $date_fin = new \DateTime();
        $date_fin->setDate($date->format('Y'), $date->format('m'), $date->format('t'));
    ...
    public function getHourRecup($id, \DateTime $date) {
        ...
        for ($i = 0; $i < $nombres_mois; $i++) {
            ...
            $heures_a_ajouter = \Model_My_Document::getHeuresAEffectuer($date_boucle, $id, 1);
