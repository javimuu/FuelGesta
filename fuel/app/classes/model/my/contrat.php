...
//        }
    ...
    public static function getHourPrecedenteContrat($date,$id_contrat){

        $date = \DateTime::createFromFormat('d/m/Y',$date);

        $pdo = \Maitrepylos\Db::getPdo();
        $sql = 'SELECT count(id_heures) as compteur FROM heures
                WHERE d_date < ?
                AND contrat_id = ?
                UNION
                SELECT 0';
        ...
        $r = $pdo->prepare($sql);
        $r->execute(array($date->format('Y-m-d'),$id_contrat));
        $set = $r->fetchAll(\PDO::FETCH_OBJ);
    ...
    public static function getHourSuivantContrat($date, $id_contrat)
    {
